...
<template>
  <div
    class="w-full h-full bg-[#252525] tracks-list select-none"
  ...
  >
    ...
    <div
      ...
      :key="track.id"
      class="relative h-12 my-3 bg-[#252525]"
      :data-track="track.id"
      @dragenter.prevent="handleTrackDragEnter(index)"
    ...
    >
      ...
      <div
        ...
        :class="{
          'ring-2 ring-[#b534cf]':
            clip.selected ||
            (hoveredClip && hoveredClip.id === clip.id) ||
        ...
        :data-clip="clip.id"
        @mousedown.stop="handleClipClick($event, clip, track.id)"
        @mouseover="handleClipHover(clip)"
        @mouseleave="handleClipLeave(clip)"
      >
        <!-- 片段内容 -->
...
const handleKeyDown = (event: KeyboardEvent) => {
  ...
  if (event.key === 'Delete' || event.key === 'Backspace') {
    props.tracks.forEach((track, trackIndex) => {
      const selectedClips = track.clips.filter(
        (clip) => clip.selected || clip === selectedClip.value
      );
