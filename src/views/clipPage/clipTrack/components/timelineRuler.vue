...
const playheadPosition = computed(
  () => props.currentTime * BASE_TICK_SPACING * props.zoom * 10
);
...
const startDragPlayhead = (e: MouseEvent) => {
  ...
  const time = Math.max(
    0,
    Math.min(mouseX / (BASE_TICK_SPACING * props.zoom * 10), props.duration)
  );
...
const handleAutoScroll = () => {
  ...
  if (scrollDirection.value) {
    emit(
      'scroll',
      scrollDirection.value === 'right' ? SCROLL_SPEED : -SCROLL_SPEED
    );
